...
print("1kb")
DNAaccessibility_tissue_1kb = sapply(unique(meta$Experiment.accession), function(e){
   print(e)
   sub = meta[meta$Experiment.accession == e,]
   if(nrow(sub) > 1){
      sub = sub[sub$Assembly == "hg19",]
   ...
   t = read.table(file = paste0("data/procData/", tissue, 
                                "/DNAaccessibility/",f,"_1kbp.out.bed"),
                  as.is = T,sep = "\t", na.strings=".")
   return(t[,ncol(t)])
})
DNAaccessibility_tissue_1kb = rowMeans(log(DNAaccessibility_tissue_1kb + 1), na.rm = T)
...
save(DNAaccessibility_tissue, DNAaccessibility_UCSC, DNAaccessibility_tissue_1kb,
