...
lapply(tissues[-grep(tissue_model, tissues)], function(tissue_apply){
  ...
  allROC_concat <- sapply(tissue_comp, function(tissue){
    if (tissue == tissue_apply){
      load(paste0(path_apply, "/ROC_CV_pCutOff",pCutOff,".RData"))     
      return(ROC_concat)
    } else {
      paste0(path_model, "/ROC_concat_modelON",tissue_apply,".RData")
      return(ROC_concat)
    }
  })
  ...
  allPR_concat <- sapply(tissue_comp, function(tissue){
    if (tissue == tissue_apply){
      load(paste0(path_apply, "/PR_CV_pCutOff",pCutOff,".RData"))     
      return(PR_concat)
    } else {
      paste0(path_model, "/PR_concat_modelON",tissue_apply,".RData")
      return(PR_concat)
    }
  })
...
ggsave(filename = "heatmap_AUROC_allModelsONallTissues.png", path = "/cellnet/MutationModel/fig", 
