...
png(filename = paste0(path_fig, "/ROC_PR_allAUROC_evenTPTN_CV_",tissue,".png"), width = 720*2, height = 720*2, pointsize = 22)
...
p <- ggplot(data = allAUC_gatherd, aes(chromosome, AUROC)) +
  geom_hline(yintercept = mean(allAUC_gatherd$AUROC),colour='blue',size = 0.3) +
  geom_boxplot() +    
  labs(x = "Testing chromosome",y = "AUROC") +
  theme_classic() +
...
png(filename = paste0(path_fig, "/ROC_evenTPTN_random_CV_",tissue,".png"), width = 720, height = 720, pointsize = 20)
par(cex.lab = 1.3)
