...
perfsTissueSpecific = sapply(modelTissues, function(tissue){
  ...
  load(paste0("data/MutTables/healthyTissuesWEX/", tissue, "_Muts_mapped_processed.RData")) #dat,datchroms
  # load model
...
perfsAllTissueModel = sapply(testTissues, function(tissue){
  print(tissue)
  # load healthy tissue trainingData
  load(paste0("data/MutTables/healthyTissuesWEX/", tissue, 
...
dumpVar = sapply(1:length(testTissues), function(i){
  ...
  # tissue-specific model
  if(tissue %in% modelTissues){
    lines(perfsTissueSpecific[[tissue]]$pr@x.values[[1]], lwd = 1.5,
          perfsTissueSpecific[[tissue]]$pr@y.values[[1]], col = tissueCols[tissue])
    lines(ROC_PR_RF_concat[[tissue]]$pr@x.values[[1]], lwd = 1.5,
...
# AUC general model
png(paste0("fig/healthyTissuesWEX/AUC", plotEnding, ".png"),
