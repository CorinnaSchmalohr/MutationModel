...
dumpVar = sapply(tissues, function(tissue){
  ...
  ids = meta$File.accession
  if(length(ids) == 0){ # for esophagus, no data with sufficient quality
    return(NULL)
  }
  if(length(ids)>1){
    bedToBedgraph = paste0("zcat data/rawdata/DNAseENCODE/", 
                           tissue,"/", ids, ".bed.gz", 
    ...
    comb = read.table(paste0("data/rawdata/DNAseENCODE/",tissue,"/", ids, ".bed.gz")) 
    comb = comb[comb$V1 %in% paste0("chr", 1:22),]
    gr = GRanges(seqnames=comb$V1,
                 ranges=IRanges(start=comb$V2+1, end = comb$V3), 
                 val = 1)
  ...
  gr = sort(gr)
  save(gr, file=paste0("data/predictors/DNAseENCODE/",
                       tissue,".RData"))
})
