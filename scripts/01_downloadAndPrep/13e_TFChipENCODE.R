...
dumpVar = sapply(tissues, function(tissue){
  ...
  dumpVar2 = sapply(names(filesByTargetBed), function(target){
    ...
    if(length(ids)>1){
      bedToBedgraph = paste0("zcat data/rawdata/TFChipENCODE/", 
                             tissue,"/", ids, ".bed.gz", 
                             " | cut -f1-3,9 | sort -k 1,1 -k2,2n ",
                             " > temp/", ids, ".bedgraph")
      # there are overlapping peaks in input files, therefore we have to merge them
      ...
      comb = read.table(paste0("data/rawdata/TFChipENCODE/",tissue,"/", ids, ".bed.gz")) 
      comb = comb[comb$V1 %in% paste0("chr", 1:22),]
      gr = GRanges(seqnames=comb$V1,
                   ranges=IRanges(start=comb$V2+1, end = comb$V3), 
                   val = 1)
    ...
    gr = sort(gr)
    save(gr, file=paste0("data/predictors/TFChipENCODE/",
