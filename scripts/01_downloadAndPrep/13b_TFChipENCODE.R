tissues = c("brain","breast", "colon","esophagus","kidney", "liver", "lung",
            "ovary", "prostate", "skin")
dir.create("data/predictors/TFChipENCODE/", showWarnings = F)
dumpVar = sapply(tissues, function(tissue){
  ...
  write.table(urls,file = paste0("data/rawdata/TFChipENCODE/", tissue, "/filesToDownload.txt"),
  ...
  command = do.call(c,lapply(names(filesByTargetBW), function(target){
    toUnify = paste0("data/rawdata/TFChipENCODE/", tissue, "/",
    ...
    if(length(toUnify) < 2){
      ...
      toBWCommand = paste0("lib/bedGraphToBigWig ",
                           ...
                           "data/predictors/TFChipENCODE/", target, "_", tissue, ".bigWig")
      rmCommand = paste0("rm ",
                         " temp/", target, "_", tissue, ".bedGraph",
                         " temp/", target, "_", tissue, "_liftOver.bedGraph",
                         " temp/", target, "_", tissue, "_liftOver_sorted.bedGraph",
  ...
  write.table(command,
              file = paste0("data/rawdata/TFChipENCODE/", tissue,
                            "/UnifyCommand.sh"),
              quote = F,row.names = F, col.names = F)
