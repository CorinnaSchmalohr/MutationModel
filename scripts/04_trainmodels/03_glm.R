...
predictions = lapply(chroms, function(cr){
   ...
   save(logR, file = paste0("data/Modeling/exomeTrainData/GLM/", tissue, "_", cr, ".RData"))
   ...
   logR = glm(formula = mutated ~ ., data = trainData, 
              family = binomial(link = "logit"))
   save(logR, sigFeatures, file = paste0("data/Modeling/exomeTrainData/GLM/", tissue, "_", cr, "_sig.RData"))
...
save(predictions, file = paste0("data/Modeling/exomeTrainData/GLM/", tissue, "_predictions_sig.RData"))
cat('\n')
...
save(feature_selection, file = paste0("data/Modeling/exomeTrainData/GLM/", tissue,"_CWCV_feature_selection.RData"))
cat('\n')
#####
...
trainData = cbind(dat[sigFeatures_selection], mutated = dat$mutated)
logR = glm(formula = mutated ~ ., data = trainData, family = binomial(link = "logit"))
save(logR, sigFeatures, file = paste0("data/Modeling/exomeTrainData/GLM/", tissue, "_sig.RData"))
#####
