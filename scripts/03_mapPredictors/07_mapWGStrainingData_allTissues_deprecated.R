...
pred = mapPredictors(x=tab, 
                     posFile="data/MutTables/WholeGenomeData/WGSMuts_allTissues.bed")
load(paste0("data/MutTables/WholeGenomeData/WGSMuts_allTissues.RData"))

data = list(meta = tab, pred = pred, muts = Muts)
save(data, file = paste0("data/MutTables/WholeGenomeData/WGSMuts_allTissues_mapped.RData"))
dat = cbind(data$pred, mutated = as.factor(data$muts$mutated))
datchroms = data$muts$chr
dat = as.data.frame(dat)
save(dat,datchroms, file=paste0("data/MutTables/WholeGenomeData/WGSMuts_allTissues_mapped_processed.RData"))
cat("\n")
