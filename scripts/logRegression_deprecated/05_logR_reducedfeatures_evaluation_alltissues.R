...
lapply(tissues, function(tissue){  
  ...
  ggplot(yhat_compare, aes(x=label, y=prediction, fill = pcutoff), ylim = c(0,1)) + 
    ...
    scale_fill_manual(values=c("#56B4E9", "#E69F00", "#999999"), name = "p-Value cutoffs", labels = c("0.01", "0.05", "not filtered"))+ 
    theme_classic() +
    theme(text = element_text(size=18), panel.grid.major = element_line(size = 0.3, linetype = 'solid', colour = "lightgrey"), 
