...
#  save(pValues, file = paste0(path, "/pValues_allChr.RData"))
  ...
  save(pValues_adjust, file = paste0(path, "/pValues_allChr_adjust.RData"))
    ...
    # Get all p-values for all factors of each feature
  pValue_factors <- sapply(chroms, function(chrom){
    print(chrom)
    
    load(paste0(path, "/", chrom,"/logR_", chrom,".RData")) 
    
    return(as.matrix(coef(summary(logR))[,4]))
  })
  rownames(pValue_factors) <- rownames(coefficients)
  pValue_factors <- as.data.frame(pValue_factors)
  save(pValue_factors, file = paste0(path, "/pValues_factors_allChr.RData"))
...
lapply(tissues, function(tissue){  
  ...
  for (row in 1:nrow(coefficients)){
    if (rownames(coefficients[row,]) %in% rownames(coefficients_filteredPval)){
      ...
      Significance <- c(Significance, paste0("yes, p-value <= ", pCutOff))
    } else if (!rownames(coefficients[row,]) %in% rownames(coefficients_filteredPval)) {
      print(paste0(row, " - filtered"))
      Significance <- c(Significance, paste0("no, p-value > ", pCutOff))
    }
  }
  
  coefficients$Significance <- Significance
  ...
  ggplot(coefficients_gg, aes(predictors, coefficient)) + 
    geom_boxplot() + 
    geom_jitter(shape = 20, aes(colour = Significance))+ 
    ...
    theme(text = element_text(size=32), legend.position = c(0.2, 0.15), 
  ...
  ggsave(filename = paste0("jitter_pValsFeatures",pCutOff,"_coefs_",tissue,".pdf"), path = paste0(path_fig),
